...
use turbopack_binding::{
    turbo::{
        ...
        tasks::{RawVc, ReadRef, TransientInstance, TransientValue, TryJoinIterExt, Vc},
        tasks_fs::{source_context::get_source_context, FileLinesContent},
    },
...
pub fn format_issue(
    ...
    // TODO(turbopack) CLICKABLE PATHS
...
impl IssueReporter for ConsoleUi {
    ...
    async fn report_issues(
        ...
        for (plain_issue, id) in issues {
            ...
            issues.push(styled_issue.trim_end().to_string());
        }
