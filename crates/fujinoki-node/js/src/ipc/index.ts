...
function createIpc<TIncoming, TOutgoing>(
  ...
  function send(message: any): Promise<void> {
    ...
    socket.write(new Uint8Array(length));

    return new Promise((resolve, reject) => {
      socket.write(new Uint8Array(packet), (err) => {
  ...
  function sendReady(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      socket.write(new Uint8Array(length), (err) => {
        process.stderr.write('TURBOPACK_OUTPUT_D\n');
        process.stdout.write('TURBOPACK_OUTPUT_D\n');
        if (err != null) {
          reject(err);
...
export const IPC = createIpc<unknown, unknown>(Number.parseInt(PORT, 10));
