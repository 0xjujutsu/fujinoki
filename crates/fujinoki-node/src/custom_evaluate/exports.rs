...
use turbopack_binding::{
    turbo::{
        ...
        tasks::{Completion, RcStr, TaskInput, Value, Vc},
...
async fn exports_compute(
    evaluate_context: ExportsEvaluateContext,
...
pub struct ExportsEvaluateContext {
...
#[async_trait]
impl EvaluateContext for ExportsEvaluateContext {
    type InfoMessage = ();
    type RequestMessage = ();
    ...
    fn compute(self, sender: Vc<JavaScriptStreamSender>) {
        let _ = exports_compute(self, sender);
    }
    ...
    async fn info(
        ...
        bail!("ExportsEvaluateContext does not support info messages")
    }
    ...
    async fn request(
        ...
        bail!("ExportsEvaluateContext does not support request messages")
    }
    ...
    let entry_module = asset_context
        .process(
            Vc::upcast(VirtualSource::new(
                ...
                AssetContent::file(
                    File::from(format!(
                        "import {{ getExportedValues }} from 'RUNTIME'; getExportedValues({}, () \
                         => import('INNER'))",
                        StringifyJs(&export_names)
                    ))
                    .into(),
                ),
            )),
