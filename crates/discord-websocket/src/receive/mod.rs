use anyhow::Result;
use discord_api::gateway::payload::{Payload, ReceiveEvents};
use turbopack_binding::turbo::tasks::{self as turbo_tasks, Vc};

use crate::context::WebsocketContext;

#[turbo_tasks::value_trait]
pub trait ReceiveEventHandler {
    fn dispatch(
        self: Vc<Self>,
        ctx: Vc<WebsocketContext>,
        data: Vc<ReceiveEvents>,
    ) -> Result<Vc<()>>;
}
...
pub async fn dispatch(
    payload: Vc<Payload>,
    receive_event_handler: Vc<Box<dyn ReceiveEventHandler>>,
    ctx: Vc<WebsocketContext>,
) -> Result<Vc<()>> {
    let payload = &*payload.await?;
    let data: ReceiveEvents = payload
        .clone()
        .d
        .expect("failed to retrieve payload data")
        .try_into()
        .expect("payload data is not a receive event");

    // TODO do something with collecting side effects from the result of this at
    // fujinoki-websocket
    Ok(receive_event_handler.dispatch(ctx, data.cell()))
    // TODO move event handling (ReceiveEventHandler) to fujinoki-websocket
    ...
    //         let ready_payload_data: ReadyPayloadData =
    // serde_json::from_value(data.clone())?;

    //         *session_id =
    // Some(data["session_id"].as_str().unwrap().to_string());
    //         *resume_gateway_url =
    // Some(data["resume_gateway_url"].as_str().unwrap().to_string());

    //         // Removes unnecessary data from the ready event, which will be
    // used when         // calling the ready event handler
    ...
    //         // * this should be faster once Turbo engine has persistent
    // caching         // if
    // resolved_source.get_commands_dir().await?.is_some() {         //
    ...
    //         if let Some(entry) = get_event_entry(resolved_source,
    // event_name.clone())             .await?
    //             .clone_value()
    //         {
    //             let debug = ctx.debug;
    ...
    //                 async move {
    //                     let project_path =
    // resolved_source.clone().await?.project_path;                     let
    // chunking_context = resolved_source                         .clone()
    //                         .await?
    //                         .executor
    //                         .await?
    //                         .chunking_context;
    ...
    //                     let env =
    // resolved_source.clone().await?.executor.await?.env;
    ...
    //     "INTERACTION_CREATE" => {
    //         let resolved_source =
    // source.resolve_strongly_consistent().await?;         let commands =
    // resolved_source.get_commands().await?;         let commands:
    // std::collections::BTreeMap<RcStr, Vc<FileSystemPath>> = commands
    //             .iter()
    //             .map(|v| (v.name.clone(), v.file_path.clone()))
    //             .collect::<BTreeMap<_, _>>();

    //         let command_name: &RcStr =
    // &data["data"]["name"].as_str().unwrap().to_string().into();
    ...
    //                     // TODO(kijv) only update this application_command
    // (route_changed for                     // invalidation) if
    // resolved_source.await?.commands_dir.await?.                     //
    ...
    //                     let SingleValue::Single(_) =
    // initial_val.await?.try_into_single().await?                     else
    // {                         // An error happened, which has already
    // been converted into an issue.                         return
    // handle_issues(                             initial_val,
    //                             issue_reporter,
    //                             IssueSeverity::Fatal.cell(),
    //                             None,
    //                             None,
    ...
    // initial_val.await?.try_into_single().await?                     else
    // {                         // An error happened, which has already
    // been converted into an issue.                         handle_issues(
    //                             initial_val,
    //                             issue_reporter,
    //                             IssueSeverity::Fatal.cell(),
    //                             None,
    ...
    //                     // TODO(kijv) classes?
    //                     // TODO(kijv) make these into functions so
    // Value::Array has no duplicated                     // code ? string =
    // pure text message                     // ? array = mix of any of the
    // types with meaning (except array)                     match
    // initial_return {                         JsonValue::String(text) => {
    ...
    // tts: None,                                         content:
    // Some(text.into()),                                         embeds:
    // None,                                         allowed_mentions: None,
    //                                         flags: None,
    //                                         components: None,
    //                                         attachments: None,
    //                                     },
    ...
    //                             // Lilybird embeds in a format that the
    // Discord API                             // already accepts, so we
    // just send this object to                             // the response
    //                             if let Ok(embed) =
    // serde_json::from_value::<lilybird::Embed>(value)
    // {                                 // Although most values are the
    // same, the accepted value relies                                 // on
    // the Turbo engine for                                 // caching
    // abilities                                 let acceptable_embed =
    // Embed {                                     title:
    // embed.title.map(|str| str.into()),
    ...
    //                                             Some(EmbedVideo {
    //                                                 // Discord API
    // technically doesn't require the
    // // url                                                 // (according
    // to the docs)                                                 url:
    // Some(video.url.into()),
    // proxy_url: video
    ...
    //                                             Some(EmbedProvider {
    //                                                 name:
    // provider.name.map(|str| str.into()),
    ...
    // data: Some(InteractionCallbackData::Messages(
    // InteractionCallbackMessagesData {
    // tts: None,                                             content: None,
    //                                             embeds:
    // Some(vec![acceptable_embed]),
    // allowed_mentions: None,
    // flags: None,                                             components:
    ...
    //         // User-provided event
    //         if let Some(event_entry) = get_event_entry(resolved_source,
    // event_name.clone())             .await?
    //             .as_ref()
    //         {
    //             let project_path =
